...
#[derive(Debug)]
pub enum Cc1101AsyncError<SpiE> {
    /// Operation timeout
    TimeoutError,
...
impl<SpiE> From<Error<SpiE>> for Cc1101AsyncError<SpiE> {
...
{
    pub fn new(spi: SPI) -> Result<Self, Cc1101AsyncError<SpiE>> {
        Ok(Cc1101Async { cc1101: Cc1101::new(spi)?})
    }

    pub fn command(&mut self, command: CommandStrobe) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.command(command)?)
    }

    pub fn set_frequency(&mut self, hz: u64) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.set_frequency(hz)?)
    }

    pub fn set_deviation(&mut self, deviation: u64) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.set_deviation(deviation)?)
    }

    pub fn set_data_rate(&mut self, baud: u64) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.set_data_rate(baud)?)
    }

    pub fn enable_fec(&mut self, enable: bool) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.enable_fec(enable)?)
    }

    pub fn set_cca_mode(&mut self, cca_mode: CcaMode) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.set_cca_mode(cca_mode)?)
    }

    pub fn set_num_preamble(&mut self, num_preamble: NumPreambleBytes) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.set_num_preamble(num_preamble)?)
    }

    pub fn crc_enable(&mut self, enable: bool) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.crc_enable(enable)?)
    }

    pub fn set_chanbw(&mut self, bandwidth: u64) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.set_chanbw(bandwidth)?)
    }

    pub fn get_hw_info(&mut self) -> Result<(u8, u8), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.get_hw_info()?)
    }

    pub fn get_rssi_dbm(&mut self) -> Result<i16, Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.get_rssi_dbm()?)
    }

    pub fn get_lqi(&mut self) -> Result<u8, Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.get_lqi()?)
    }

    pub fn set_sync_mode(&mut self, sync_mode: SyncMode) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.set_sync_mode(sync_mode)?)
    }

    pub fn set_modulation(&mut self, format: Modulation) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.set_modulation(format)?)
    }

    pub fn set_address_filter(&mut self, filter: AddressFilter) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.set_address_filter(filter)?)
    }

    pub fn set_packet_length(&mut self, length: PacketLength) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.set_packet_length(length)?)
    }

    pub fn white_data(&mut self, enable: bool) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.white_data(enable)?)
    }

    pub fn read_tx_bytes(&mut self) -> Result<u8, Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.read_tx_bytes()?)
    }

    pub fn read_rx_bytes(&mut self) -> Result<u8, Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.read_rx_bytes()?)
    }

    pub fn read_machine_state(&mut self) -> Result<MachineState, Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.read_machine_state()?)
    }

    pub async fn check_machine_state(&mut self, target_state: MachineState) -> Result<(), Cc1101AsyncError<SpiE>> {
    ...
    pub async fn await_machine_state(&mut self, target_state: MachineState, timeout: Duration<u64, 1, 1000>) -> Result<(), Cc1101AsyncError<SpiE>> {
    ...
    pub fn read_data(&mut self, data: &mut [u8]) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.read_data(data)?)
    }

    pub fn write_data(&mut self, data: &mut [u8]) -> Result<(), Cc1101AsyncError<SpiE>> {
        Ok(self.cc1101.write_data(data)?)
    }

    /// Set Radio Mode.
    pub async fn set_radio_mode(&mut self, radio_mode: RadioMode, timeout: Duration<u64, 1, 1000>) -> Result<(), Cc1101AsyncError<SpiE>> {
