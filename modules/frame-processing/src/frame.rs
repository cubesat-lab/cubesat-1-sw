    ...
    if *buffer_size >= FRAME_HEADER_MIN_LENGTH 
    {
        ...
        if buffer[FRAME_FIRST_BYTE] == FRAME_START && buffer[FRAME_SECOND_BYTE] == FRAME_START 
        {
            ...
            if *buffer_size >= data_len + FRAME_HEADER_MIN_LENGTH
            {
                ...
                let frame_crc = ((buffer[FRAME_BEGIN_LENGTH + data_len] as u16) << 8) | buffer[FRAME_BEGIN_LENGTH + data_len + 1] as u16;

                let nb_bytes_length = 2;
                let length_offset = 2;
                let data_for_crc = &buffer[nb_bytes_length..nb_bytes_length + length_offset + data_len];
